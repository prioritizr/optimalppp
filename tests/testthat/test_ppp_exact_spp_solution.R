context("ppp_exact_spp_solution")
...
test_that("solution (single solution, no constraints)", {
  ...
  s <- ppp_exact_spp_solution(project_data, action_data, spp, 0.18, "name",
...
test_that("solution (single solution, locked in + out constraints)", {
  ...
  spp <- tibble::tibble(name = c("S1", "S2", "S3"), weight = c(5, 5, 1))
  tree <- star_phylogeny(spp[[1]], spp[[2]])
  s <- ppp_exact_spp_solution(project_data, action_data, spp,
                              2, "name", "success", "name", "cost", "name",
...
test_that("solution (single solution, tricky problem)", {
  ...
  spp <- tibble::tibble(name = c("S1", "S2"), weight = c(5, 5))
  tree <- star_phylogeny(spp[[1]], spp[[2]])
  s <- ppp_exact_spp_solution(project_data, action_data, spp, 1.0, "name",
                              "success", "name", "cost", "name", "weight")
  # tests
  ## class
  expect_is(s, "tbl_df")
...
test_that("solution (multiple solutions, no constraints)", {
  ...
  ## statistics columns
  expect_equal(s$budget, rep(0.18, nrow(s)))
...
test_that("solution (multiple solutions, locked in + out constraints)", {
  ...
  expect_warning(s <- ppp_exact_spp_solution(project_data, action_data, spp,
...
test_that("invalid arguments", {
  ...
  expect_is(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                   sim_species_data, 1000, "name", "success",
                                   "name", "cost", "name", "weight"), "tbl_df")
  # invalid names
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
    ppp_exact_spp_solution(sim_project_data, sim_action_data,
                           sim_species_data, 100, "name1", "success", "name",
                           "cost", "name", "weight")
  })
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
    ppp_exact_spp_solution(sim_project_data, sim_action_data,
                              sim_species_data, 100, "name", "success1", "name",
                              "cost", "name", "weight")
  })
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
    ppp_exact_spp_solution(sim_project_data, sim_action_data,
                              sim_species_data, 100, "name", "success", "name1",
                              "cost", "name", "weight")
  })
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
    ppp_exact_spp_solution(sim_project_data, sim_action_data,
                              sim_species_data, 100, "name", "success", "name",
                              "cost1", "name", "weight")
  })
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
  ...
  # invalid budget
  data(sim_project_data, sim_action_data, sim_species_data)
  expect_error(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                        sim_species_data, NA_real_,
                                        "name", "success", "name", "cost",
                                        "name", "weight"))
  expect_error(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                        sim_species_data, "A", "name",
                                        "success", "name", "cost", "name",
                                        "weight"))
  expect_error(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                        sim_species_data, -5, "name", "success",
                                        "name", "cost", "name", "weight"))
  # invalid success
  expect_error({
    data(sim_project_data, sim_action_data, sim_species_data)
    sim_project_data$success[1] <- NA_real_
    ppp_exact_spp_solution(sim_project_data, sim_action_data, sim_species_data,
  ...
  # gap
  expect_error(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                      sim_species_data, 200, "name", "success",
                                      "name", "cost", "name", "weight",
                                      gap = -1))
  expect_error(ppp_exact_spp_solution(sim_project_data, sim_action_data,
                                      sim_species_data, 200, "name", "success",
