context("ppp_heuristic_phylo_solution")
...
test_that("single solution, no constraints", {
  ...
  s <- ppp_heuristic_phylo_solution(project_data, action_data, tree, 0.18,
...
test_that("single solution, zero budget", {
  ...
  tree <- ape::read.tree(text = "((S1,S2),S3);")
  tree$edge.length <- c(100, 5, 5, 5)
  s <- ppp_heuristic_phylo_solution(project_data, action_data, tree, 0,
                                    "name", "success", "name", "cost")
  # tests
  ## class
  expect_is(s, "tbl_df")
...
test_that("multiple solutions, cost limit", {
  ...
  tree <- ape::read.tree(text = "((S1,S2),S3);")
  tree$edge.length <- c(100, 5, 5, 5)
  expect_warning(s <- ppp_heuristic_phylo_solution(project_data, action_data,
                                                   tree, 0.15, "name",
                                                   "success", "name",
                                                   "cost",
...
test_that("invalid arguments", {
  ...
  expect_is(ppp_heuristic_phylo_solution(sim_project_data, sim_action_data,
                                         sim_tree, 1000, "name", "success",
                                         "name", "cost"), "tbl_df")
  # invalid budget
  expect_error(ppp_heuristic_phylo_solution(
  ...
  expect_error({
    ...
    ppp_heuristic_phylo_solution(sim_project_data, sim_action_data, sim_tree,
