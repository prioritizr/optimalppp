context("ppp_random_spp_solution")
...
test_that("single solution, no constraints)", {
  ...
  s <- ppp_random_spp_solution(project_data, action_data, spp, 0.18,
...
test_that("single solution, zero cost)", {
  ...
  spp <- tibble::tibble(name = c("S1", "S2", "S3"), weight = c(5, 5, 1))
  tree <- star_phylogeny(spp[[1]], spp[[2]])
  s <- ppp_random_spp_solution(project_data, action_data, spp, 0,
                               "name", "success", "name", "cost", "name",
                               "weight")
  # tests
  ## class
...
test_that("single solution, locked in constraints)", {
  ...
  s <- ppp_random_spp_solution(project_data, action_data, spp, 0.18,
                               "name", "success", "name", "cost", "name",
                               "weight", locked_in_column_name = "locked_in")
  # tests
  ## class
  expect_is(s, "tbl_df")
  expect_equal(ncol(s), 10L)
...
test_that("single solution, locked out constraints)", {
  ...
  s <- ppp_random_spp_solution(project_data, action_data, spp, 0.18,
                               "name", "success", "name", "cost", "name",
                               "weight", locked_out_column_name = "locked_out")
  # tests
  ## class
  expect_is(s, "tbl_df")
  expect_equal(ncol(s), 10L)
